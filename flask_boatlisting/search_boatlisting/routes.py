...
def echo():
    ...
    if request.method == "POST":
        ...
        if sortparam == 'Location':
            def keyparam(s): return s['Location']
        elif sortparam == 'Price':
            #keyparam = lambda s: float('inf') if s['Price'].lower() == "call for price" else int(s['Price'].replace(',', ''))
            def keyparam(s): return int(s['Price'].replace(',', '')) if s['Price'].replace(
                ',', '').replace('-', '').isdigit() else float('inf')
        elif sortparam == 'Size':
            def keyparam(s): return s['Size']
    ...
    # SET DEFAULTS
        ...
        searchparameters = boatsearchinput(
            sitename, inputcurr, minprice, maxprice, minlength, maxlength, sortparam)
    ...
    # CALL THE TWO FUNCTIONS!!!!!
        if sitename == "SBL":
            ...
            listsite, boatcount = sailboatlisting_loop(
                searchparameters, urljson, arrayjson)
        elif sitename == "YW":
            listsite, boatcount = yachtworld_loop(
                searchparameters, urljson, arrayjson)
        elif sitename == "both":
            listsite, boatcountSBL = sailboatlisting_loop(
                searchparameters, urljson, arrayjson)
            listsite, boatcountYW = yachtworld_loop(
                searchparameters, urljson, arrayjson)
    ...
    # Add Preface list (array)
        ...
        with open('search_boatlisting/output/boatlist.json', 'w') as outfile:
            ...
            json.dump({'fileinfo': arraypreface, 'boats': arrayjson},
                      outfile, indent=4, default=str)
